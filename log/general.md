...
## [2026-07-05 12:00] AI開発ガイドライン一般化テンプレート (AGENTS_template.md) の最終ガバナンス調整

### 1. 作業概要
- ユーザーからの最終レビューフィードバックに基づき、`AGENTS_template.md` のガバナンスと安全対策を更に強化。
- **見出し番号の整理 (1.4a → 1.5)**:
  - 枝番 `1.4a` を整数・小数形式の `1.5` に改め、それ以降のセクション番号を綺麗に繰り下げて整合性を確保。
- **変更禁止ディレクトリの重要タグ追加 (2.4)**:
  - プロジェクト固有の変更禁止ファイル・ディレクトリの定義に「(重要)」タグを追加し、1.0 で定めた最優先ガードレール（ユーザーの直近指示で安易に上書きされない制限）に正しく適合するよう修正。
- **自律修正の試行上限設定 (1.1)**:
  - 同一エラーに対する無限の自律修正ループやファイルの無駄な破壊を防ぐため、試行回数上限（例: 3回）で自律実行を停止しユーザーにエスカレーションするルールを追加。
- **破壊的操作・本番影響操作のガードルール追加 (1.9)**:
  - Git以外の操作（コンテナ停止・再起動、ファイル削除、DBマイグレーションやレコード削除、外部サービス等）を事前承認制（重要項目）にし、必要に応じてドライランの提示を求める仕組みを新設。

### 2. 変更・追加されたファイル
| ファイル | 深刻度 | 変更内容 |
|---------|--------|---------|
| [.agents/AGENTS_template.md](file:///workspaces/pole/.agents/AGENTS_template.md) | 🟢 修正 | 構成整理、変更禁止への重要タグ付与、自律修正試行回数上限の導入、破壊的操作への確認ガードの追加。 |

---

## [2026-07-05 11:42] AI開発ガイドライン一般化テンプレート (AGENTS_template.md) のガバナンス強化と最終改訂

### 1. 作業概要
- ユーザーからのガバナンス面に関する追加レビューに基づき、AIエージェントの暴走や機密情報漏洩を防ぎ、一貫した運用を可能にするための項目を追加。
- **指示の優先順位ルール (1.0)**:
  - ユーザーの直近の指示を優先しつつ、Git制限やセキュリティなど「(重要)」項目は明示的な一時解除がない限り本テンプレートの規定を優先する基本原則を追加。
- **承認可否の境界定義 (1.1)**:
  - 自律実行可能（バグ修正、コメント追加等）な範囲と、承認必須（新規モジュール作成、インターフェース変更、設定変更等）の具体例を記載するプレースホルダーを追加。
- **機密情報の独立ルール化 (1.7)**:
  - APIキーやシークレットを平文で記録しないこと、および環境変数等を経由することを示すセキュリティ項目を「(重要)」として独立セクション化。
- **完了報告のフォーマット統一 (1.4a)**:
  - 作業完了時の要約、意図、検証結果、残課題をまとめた定型フォーマットを追加。
- **指示ドリフト対策 (1.4)**:
  - 長時間セッションや多数のツール呼び出し時に、定期的に本ファイルを参照し自己逸脱をチェックするルールを追加。
- **変更履歴の追加 (6.)**:
  - 本ファイル自体の変更履歴をバージョン管理するためのセクションを追加。

### 2. 変更・追加されたファイル
| ファイル | 深刻度 | 変更内容 |
|---------|--------|---------|
| [.agents/AGENTS_template.md](file:///workspaces/pole/.agents/AGENTS_template.md) | 🟢 修正 | 優先順位、承認境界、セキュリティ、報告フォーマット、ドリフト対策、変更履歴の追加によるガバナンス面の大幅な強化。 |

---

## [2026-07-05 11:35] AI開発ガイドライン一般化テンプレート (AGENTS_template.md) の再レビュー反映と最終改訂

### 1. 作業概要
- ユーザーからの再レビューフィードバックに基づき、`AGENTS_template.md` の細部および最終的な一般化を完了。
- **残りのドメイン依存語の一般化と抽象化**:
  - 1.3 の作業ログ記述例に残っていた「デッキ別（log/<デッキ名>.md）」という表記を、汎用的な「モジュール別（log/<モジュール名>.md）」へ修正。
  - 冒頭文の例示「シニアPythonエンジニア」を「シニア[開発言語]エンジニア」へ抽象化。
  - 1.2 内に紛れ込んでいた「既存 of ソースコード」を「既存のソースコード」へ修正。
- **認証情報ルールの追加 (3.2)**:
  - ローカル検証時に必要な認証情報やアクセストークン等の配置方法・ファイルパスを記述するためのプレースホルダーを追加。
- **表現の冗長性改善 (1.3)**:
  - 作業ログの記録箇所にて「ログの管理は〜に分割して管理します」と「管理」が重複していた表現を、「ログは〜に分割して管理してください」に改善。
- **コードブロック言語の抽象化 (3.1)**:
  - テストコードの例のコードブロックを ` ```python ` 固定から ` ```[使用言語] ` のようにプレースホルダー化し、言語非依存のテンプレートとしての整合性を向上。

### 2. 変更・追加されたファイル
| ファイル | 深刻度 | 変更内容 |
|---------|--------|---------|
| [.agents/AGENTS_template.md](file:///workspaces/pole/.agents/AGENTS_template.md) | 🟢 修正 | 認証情報ルールの追加、冗長表現の改善、コードブロック言語の抽象化、ドメイン用語の更なる一般化と残りの「of」誤字修正を適用。 |

---

## [2026-07-05 11:30] AI開発ガイドライン一般化テンプレート (AGENTS_template.md) のレビュー反映と改訂

### 1. 作業概要
- ユーザーからのレビューフィードバックに基づき、`AGENTS_template.md` を改訂。
- **誤字修正**:
  - 冒頭の説明文内の「チーム共同開発 of プロセス」を「チーム共同開発のプロセス」に修正。また、本文中に混入していたその他「of」の誤字（解説 of 提供、探索ロジック of 設計方針）をすべて「の」に修正。
- **抜け落ちていた汎用概念の復元と一般化**:
  - **コード品質 (1.2)**: 暫定的な静的解析スキップ手段（例: `--no-lint`）の扱いと、最終的に通過必須とするルールの一般化。
  - **ドキュメント管理・作業ログ (1.3)**: ログの機能・対象別での分割管理（共通と個別）の構成プレースホルダー化。ガイドライン自己言及の文言（「本ガイドライン読み込み後」）の復元。
  - **定期レビュー (1.4)**: セルフレビューの手法（サブエージェント、別書き出し、コード再読など）の自由度の記述の一般化。
  - **開発環境・Git制限 (1.5)**: 特定環境下で特定のGit操作が失敗した場合の代替手段（ホスト側実行など）の案内フローの追加。
  - **変更禁止 (2.4)**: 完全保護対象（変更禁止）と準保護対象（慎重に扱うべき共有コア）の二層管理への整理。
  - **テスト実行 (3.1)**: 検証用スクリプトの雛形や実行コードブロックの記述用プレースホルダーの設置。
  - **検証ルール (3.2)**: ハードコーディング禁止、複数構成の管理、共通リソース同期の仕組み（update_cg.py相当）、自動パッチ動作の注意点などの一般化。
  - **CI/CD (4.1)**: 「開発成果物の自動整理・移動（アーカイブ）」と「リリース物の自動生成・提出」を別の工程として分離（全5項目化）。
- **プレースホルダーの表記統一**:
  - `[...を記述。例: ...]` のスタイルに統一し、表記ゆれを解消。

### 2. 変更・追加されたファイル
| ファイル | 深刻度 | 変更内容 |
|---------|--------|---------|
| [.agents/AGENTS_template.md](file:///workspaces/pole/.agents/AGENTS_template.md) | 🟢 修正 | レビュー指摘を反映し、誤字の修正、各種汎用ルールの復活、プレースホルダー表記の統一を含む全面的な改訂を実施。 |

---

## [2026-07-04 14:38] AI開発ガイドライン一般化テンプレート (AGENTS_template.md) の新規作成

### 1. 作業概要
- 他プロジェクトでも再利用できるように、現在の `AGENTS.md` からプロジェクト固有のルール（ポケモンカードゲーム API、Kaggle 環境、検証用スクリプトなど）をプレースホルダー化して排除し、協働の基本原則（コミュニケーション、確認フロー、Git操作制限、作業ログ等）のみを抽出した一般化テンプレートを作成。
- **新規ファイルの追加**:
  - [.agents/AGENTS_template.md](file:///workspaces/pole/.agents/AGENTS_template.md) を新設。

### 2. 変更・追加されたファイル
| ファイル | 深刻度 | 変更内容 |
|---------|--------|---------|
| [.agents/AGENTS_template.md](file:///workspaces/pole/.agents/AGENTS_template.md) | 🟢 新規 | プロジェクト固有ルールをプレースホルダー化し、協働ルールのみを抽出したAI開発ガイドラインテンプレートを新規追加。 |
