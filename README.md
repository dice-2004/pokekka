...
コードの変更、エージェントの新規追加、パラメータの調整を行った際は、チーム内の開発整合性を保つため、必ず **[log/](log/)** ディレクトリ配下の適切な作業ログファイル（共通インフラの変更は `log/general.md`、各デッキの変更は `log/<デッキ名>.md`）の最上部に「作業内容」「実装の意図」「検証結果」を記録してください。
