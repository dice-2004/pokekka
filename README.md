...
├── sample_submission/     # 提出用パッケージのソースディレクトリ
│   ├── main.py            # エージェントのメインロジック (提出対象)
│   ├── deck.csv           # デッキ構成ファイル (提出対象)
│   └── cg/                # シミュレータ API / 共有ライブラリ (提出対象)
├── tests/                 # ローカル検証・CI用テストスクリプト
│   ├── dry_run.py         # 動作検証テスト (1ゲームエラーなく完走するか)
│   ├── benchmark.py       # 勝率測定ベンチマークテスト (自動パッチ・エイリアス内蔵)
...
```
*   **引数**:
    *   `--agent-a`: テストしたい新エージェントのパス
    *   `--agent-b`: 比較対象（ベースラインなど）のエージェントのパス
    *   `--matches`: 対戦回数 (先攻後攻が均等になるよう偶数、通常は 20〜50 を推奨)
*   **特徴**: 環境内の古いゲームエンジンを最新版で自動上書きするパッチ機能、ライブラリの二重ロード防止機能を内蔵しており、提出用コードを一切汚さずに動作します。
...
## 7. 別エージェント作成方法
sample_submission/main.pyをコピーして、別のエージェントを作成可能
