...
def _effect_target_score(
    pokemon: Pokemon,
    option_player_index: int,
    my_index: int,
) -> int:
    if option_player_index != my_index:
        upper_damage = _effective_damage(STARMIE_UPPER_ATTACK_ID, pokemon)
        score = pokemon_score(pokemon)

        if upper_damage >= pokemon.hp:
            score += 25000 + (200 - pokemon.hp) * 20
        elif pokemon.hp <= 50:
            score += 12000 + (50 - pokemon.hp) * 10
        else:
            score += max(0, 5000 - pokemon.hp * 20)
    ...
    if pokemon.id == Mega_Starmie_ex and pokemon.hp < pokemon.maxHp:
        return 8000
    if pokemon.id == Staryu:
        return 3000
    return pokemon_score(pokemon)
            ...
            if card is not None:
                ...
                if context in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
                    ...
                    if isinstance(card, Pokemon):
                        score = _effect_target_score(card, option.playerIndex, my_index)
                    else:
                        score = 0
            ...
            if card is not None and isinstance(pokemon, Pokemon):
                ...
                if card.id == Basic_Water_Energy and pokemon.id == Staryu:
                    ...
                    if option.inPlayArea == AreaType.ACTIVE:
                        bench_staryu_count = sum(
                            1
                            for bench_pokemon in my_state.bench
                            if bench_pokemon is not None and bench_pokemon.id == Staryu
                        )
                        if bench_staryu_count > 0:
                            score -= 1800
                    elif option.inPlayArea == AreaType.BENCH:
                        score += 600
